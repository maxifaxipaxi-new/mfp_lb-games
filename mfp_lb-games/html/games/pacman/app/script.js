...
function mousePressed(){
  ...
  pressCoord = createVector(mouseX, mouseY)
  doubleTime = 30
}

function mouseDragged(){
  mCoord = createVector(mouseX,mouseY)
  if (pressCoord.dist(mCoord) > 20){
    doubleTime = -1
    mCoord.sub(pressCoord)
    dir = round(mCoord.heading() / 90)
    input = []
    if (dir == 0){
      input.push("d")
    }
...
function draw(){
  if (mouseIsPressed){
    ...
    if (gameState == "menu" && pressedTicks > 0){
      input = []
    }
    pressedTicks = 0
  }
